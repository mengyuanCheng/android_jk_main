package com.grgbanking.ct.rfid;
...
public class UfhData {
	...
	public static int LengthByte=0;
	public static String PiPeiEPC="";
	...
	public static void read6c(){
		...
		for (int i = 0; i < scaned_num; i++) {
			...
			else
			{
				if(key.length()-(5)>0)
				{
					startByte =  key.length()-5;
					String temp = key.substring(startByte,key.length());//??????????????????????
					if(temp.equals(PiPeiEPC))//???????????????????????Map
					{
						int num = scanResult6c.get(key) == null ? 0 : scanResult6c.get(key);
						scanResult6c.put(key, num + 1);//
					}
